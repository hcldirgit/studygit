...
do
    case $mysql_v in
        ...
        5.6)
            cd /usr/local/src
            [ -f mysql-5.6.34-linux-glibc2.5-$ar.tar.gz ] || wget http://mirrors.sohu.com/mysql/MySQL-5.6/mysql-5.6.34-linux-glibc2.5-$ar.tar.gz
            tar zxf mysql-5.6.34-linux-glibc2.5-$ar.tar.gz
            check_ok
            [ -d /usr/local/mysql ] && /bin/mv /usr/local/mysql /usr/local/mysql_bak
            mv mysql-5.6.34-linux-glibc2.5-$ar /usr/local/mysql
            if ! grep '^mysql:' /etc/passwd
            then
                useradd -M mysql -s /sbin/nologin
            fi
...
cd /usr/local/src
[ -f httpd-2.2.16.tar.gz ] || wget  http://mirrors.cnnic.cn/apache/httpd/httpd-2.2.31.tar.gz
...
wget http://www.apelearn.com/study_v2/.nginx_init  && /bin/mv .nginx_init /etc/init.d/nginx
check_ok
chmod 755 /etc/init.d/nginx
chkconfig --add nginx
chkconfig nginx on
wget http://www.apelearn.com/study_v2/.nginx_conf && /bin/mv .nginx_conf /usr/local/nginx/conf/nginx.conf
check_ok
service nginx start
check_ok
...
do
    case $php_v in
        5.4)
            ...
            [ -f /usr/local/php-fpm/etc/php-fpm.conf ] || wget http://www.apelearn.com/study_v2/.phpfpm_conf 
            /bin/mv .phpfpm_conf /usr/local/php-fpm/etc/php-fpm.conf
